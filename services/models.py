...
class TenderTimeline(models.Model):
    ...
    tender = models.OneToOneField(
        "Tender", on_delete=models.CASCADE, related_name="timeline"
    )
...
class Tender(models.Model):
    TENDER_STATUS = [
        ("draft", "Draft"),
        ("pending_superuser_approval", "Pending Superuser Approval"),
        ("rejected_by_superuser", "Rejected by Superuser"),
        ("pending_manager_assignment", "Pending Manager Assignment"),
    ...
    status = models.CharField(max_length=30, choices=TENDER_STATUS, default="draft")
    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="tenders_created"
    )
    company = models.ForeignKey(Company, on_delete=models.CASCADE)
    ...
    # Manager assignment fields - Updated with through_fields
    assigned_to = models.ManyToManyField(
        User,
        related_name="assigned_tenders",
        blank=True,
        through="TenderManagerAssignment",
        through_fields=("tender", "manager"),  # Specify the fields to use
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if self.required_department:
            managers = User.objects.filter(
                department=self.required_department, role="manager"
            )
...
class TenderManagerAssignment(models.Model):
    tender = models.ForeignKey(Tender, on_delete=models.CASCADE)
    manager = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="tender_assignments"
    )
    ...
    def __str__(self):
        return f"{self.tender.reference_number} - {self.manager.email}"

    def save(self, *args, **kwargs):
        # Deactivate other assignments when creating a new one
        if self.is_active:
            TenderManagerAssignment.objects.filter(
                tender=self.tender, is_active=True
...
class Document(models.Model):
    ...
    tender = models.ForeignKey(
        Tender, on_delete=models.CASCADE, related_name="documents"
    )
...
class Approval(models.Model):
    ...
    tender = models.ForeignKey(
        Tender, on_delete=models.CASCADE, related_name="approvals"
    )
...
class ChecklistItem(models.Model):
    ...
    tender = models.ForeignKey(
        Tender, on_delete=models.CASCADE, related_name="checklist_items"
    )
    ...
    document = models.FileField(upload_to="checklist_documents/", null=True, blank=True)
    comments = models.TextField(blank=True, null=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
